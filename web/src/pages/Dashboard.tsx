 ...
 type StatusFilter = 'all' | 'done' | 'pending' | 'doing';
 
 const Dashboard = () => {
   const navigate = useNavigate();
   const { cards, addMultipleCards } = useCardsStore();
   ...
   const handleCardsGenerated = (newCards: Card[]) => {
     ...
     toast.success(`${newCards.length} card${newCards.length > 1 ? 's' : ''} criado${newCards.length > 1 ? 's' : ''} com sucesso!`);
   };
     ...
     <DashboardLayout>
       <div className="space-y-6">
         ...
         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           ...
           <Button onClick={() => navigate('/cards/new')} className="gradient-primary">
